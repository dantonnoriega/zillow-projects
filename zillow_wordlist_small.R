...
ss <- data.table(matrix(unlist(strsplit(s$ngram, split = "\\s")), ncol = 2, byrow = TRUE))
setnames(ss, old = names(ss), new=c("b1","b2"))
attach(ss)
woi <- rbind(s[b1 %in% z.woi$ngram, ], s[b2 %in% z.woi$ngram, ])
woi <- unique(x)

setwd("/Users/dnoriega/Dropbox/SolarHedonic/Dan/")
