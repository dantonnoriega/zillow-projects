...
if (`toggle' == 1) {
	...
	* load data, merge single family home info
	use "D:\Dan's Workspace\Zillow\data\data_wo_descriptions.dta", clear
	merge m:1 propertyid using "D:\Dan's Workspace\Zillow\data\type.dta"
	pause 

	* keep single family residences ("sfr")
	keep if _merge == 3
	keep if definition == "sfr"
	...
	**** (1)	
	...
	removesym streetname streetdirectionsuffix streetdirectionprefix streetsuffix, spanish basicsym numbers
	...
	save "D:\Dan's Workspace\Zillow\data\data_removesym", replace
	pause
	
}

else {
	use "D:\Dan's Workspace\Zillow\data\data_removesym", clear
	tempfile hold
	save `hold', replace
}
...
removesym housenumber postalcode zipplusfour, basicsym let quotes
...
removesym postalcode zipplusfour housenumber, basicsym let quotes // remove all non-numbers
...
* save the refined, but suspicious, houses
tempfile prblmrfd
...
* save removed properties if they exist
count
local N = `r(N)'
